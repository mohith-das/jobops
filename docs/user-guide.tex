% job_ops-mcp — End-to-end user guide.
...
\fancyhead[L]{\small\sffamily\color{accent!85!black} job\_ops-mcp \textemdash{} user guide}
...
% PDF metadata set post-load so the underscore in "job_ops-mcp" survives.
\hypersetup{
...
% ── Macros ──────────────────────────────────────────────────────────────────
\newcommand{\pkg}{\texttt{job\_ops-mcp}}
...
\vspace*{2cm}
\begin{tikzpicture}
  \node[font=\Huge\sffamily\bfseries\color{accent}] (t) at (0,0) {job\_ops-mcp};
...
\begin{tabular}{rl}
\textsf{\small\bfseries Version} & \texttt{0.8.1} \\
\textsf{\small\bfseries Repo}    & \url{https://github.com/mohith-das/job_ops-mcp} \\
...
One process. Three planes. Six MCP resources and 38 tools.
...
% Storage layer
\node[store] (sqlite) at (2.2, 1.0) {SQLite\\\scriptsize\texttt{data/mcp-jsa.db}\\\scriptsize WAL mode};
...
\begin{itemize}
\item \textbf{Boot in 2 seconds.} \cmd{npx job\_ops-mcp start} \textemdash{} no docker compose, no migrations to run by hand, no init container.
...
\begin{tipbox}
The CLI is also published as the dual alias \cmd{npx job-ops-mcp} (hyphen-only). They are the same binary \textemdash{} use whichever your fingers prefer.
\end{tipbox}

\begin{tipbox}[The edit-cv.md loop]
Your \tool{career\_packet} is the source of truth the chat uses to score JDs and
draft materials. It's built from \fpath{cv.md} + \fpath{config/profile.yml}. Whenever
you edit \fpath{cv.md}, \textbf{re-run reseed} so the chat sees the new bullets.
\cmd{reseed} writes a NEW active version (previous version demoted, history kept).
The chat can also call the \tool{reseed\_career\_packet} MCP tool to do the same
thing without leaving the conversation. \cmd{doctor} warns when \fpath{cv.md}
changed since the last reseed.
\end{tipbox}

\begin{warnbox}[Source of truth — avoid packet drift]
The active packet in the DB is \emph{runtime state}; \fpath{cv.md} + \fpath{config/profile.yml}
(+ \fpath{modes/career\_packet.md}) are the \emph{source of truth}. Edit the source files and
reseed \textemdash{} do \textbf{not} hand-edit Sections 1--8 of the packet directly, because the
next reseed regenerates them from the source files and silently drops DB-only edits (the
classic ``drift'' trap). Identity / links / taglines $\rightarrow$ \fpath{config/profile.yml};
experience / projects / skills / education $\rightarrow$ \fpath{cv.md}; standing policy with no
CV/profile field (naming conventions, rendering rules, custom guardrails) $\rightarrow$
\fpath{modes/career\_packet.md} Section 9, which reseed preserves. A single self-contained
operator reference also ships at \fpath{docs/PROJECT\_MEMORY.md} \textemdash{} drop it into your
MCP client's project memory.
\end{warnbox}

\subsection*{What \tool{init} creates}

After \cmd{npx job\_ops-mcp init} from a clean directory you'll have:
...
`-- data/
    `-- mcp-jsa.db         <- SQLite with both migrations applied
\end{lstlisting}
...
\midrule
\env{MCP\_JSA\_PORT}              & HTTP port. \textit{(7891)} \\
\env{MCP\_JSA\_HOST}              & Bind host. \textit{(127.0.0.1)} \\
\env{MCP\_JSA\_PROJECT\_ROOT}     & Where \fpath{cv.md} / \fpath{profile.yml} / \fpath{portals.yml} live. \textit{(current directory)} \\
\env{MCP\_JSA\_DATA\_DIR}         & SQLite location. \textit{(\texttt{<project\_root>/data})} \\
\env{MCP\_JSA\_OUTPUT\_DIR}       & Where generated PDFs and report HTML land. \textit{(\texttt{<project\_root>/output})} \\
\env{MCP\_JSA\_VISA\_SCORING}     & Set to \texttt{false} to drop the visa surface entirely (see \S\ref{sec:custom}). \textit{(true)} \\
\env{MCP\_JSA\_PUBLIC\_BASE\_URL} & Public URL emitted in artifact links. Set when running on a remote host (Tailscale / LAN / cloud) so links are reachable from your chat machine. See \S\ref{sec:remote-host}. \textit{(default: \texttt{http://127.0.0.1:<port>})} \\
\env{MCP\_JSA\_LLM\_PROVIDER}     & \texttt{gemini} \textbar{} \texttt{deepseek} \textbar{} \texttt{none}. \textit{(gemini)} \\
\env{MCP\_JSA\_LLM\_MODEL}        & Provider-specific model id. \textit{(empty \textrightarrow{} provider default)} \\
\env{GEMINI\_API\_KEY}            & Gemini key. Needed only for \texttt{api}/batch paths. \\
\env{DEEPSEEK\_API\_KEY}          & DeepSeek key. Same. \\
\env{MCP\_JSA\_SCHEDULER\_ENABLED} & Whether opt-in cron runs at all. \textit{(false)} \\
\bottomrule
\end{tabularx}
\end{center}

\subsection*{Connecting your chat client}

\cmd{npx job\_ops-mcp connect} prints copy-paste-ready config for the common clients. Below are the two you most likely care about.

\subsubsection*{Claude Desktop (stdio transport)}
...
\env{MCP\_JSA\_PORT} in the background so the \texttt{/files/*} artifact links the
...
Restart Claude Desktop. The 35--38 tools should appear in the tool picker.
...
Boot the server separately (\cmd{npx job\_ops-mcp start}), then restart LibreChat.
...
Set \env{MCP\_JSA\_PUBLIC\_BASE\_URL} to the URL the chat machine actually uses to reach
the server:
...
binds to \env{MCP\_JSA\_HOST} (default \texttt{127.0.0.1}); to accept connections from other
devices, also set \env{MCP\_JSA\_HOST=0.0.0.0}. \cmd{npx job\_ops-mcp doctor} prints the
...
does NOT change the bind address. If you set \env{MCP\_JSA\_HOST=0.0.0.0} so the server
...
\subsection*{Optional: server-side scoring (usually no key needed)}
...
For \emph{server-side} scoring (\tool{batch\_evaluate}, or \texttt{mode: api} on tools that accept it), the server first tries \textbf{MCP sampling} \textemdash{} it asks your connected client's model directly, so \textbf{no API key is required}. A BYO key is only the fallback for clients that don't support sampling (see \S\ref{sec:mcp-features}). To configure that fallback:

\begin{lstlisting}[language=bashplus]
export MCP_JSA_LLM_PROVIDER=gemini
export GEMINI_API_KEY=...      # free tier covers ordinary single-user usage

# or
export MCP_JSA_LLM_PROVIDER=deepseek
export DEEPSEEK_API_KEY=...

# Force the BYO-key path even when the client could sample:
export MCP_JSA_SAMPLING=false
\end{lstlisting}
...
\begin{itemize}
\item \textbf{LinkedIn network} \textemdash{} download your data export at LinkedIn $\rightarrow$ Settings $\rightarrow$ Data Privacy $\rightarrow$ Get a copy of your data $\rightarrow$ Connections. Call \tool{import\_linkedin} with the path to \fpath{Connections.csv}. Unlocks \tool{find\_warm\_intros} and \tool{find\_founders}.
...
38 tools, grouped by what they do. For each: what it does, key parameters, what it returns. The default mode for tools that take one is \texttt{chat} \textemdash{} the chat client is the brain; \texttt{api} mode is the LLM-direct path and needs an API key.
...
\item[\tool{render\_pdf}] Produces resume + cover in any subset of \texttt{\{pdf, tex, docx\}} via the \texttt{formats} param (default \texttt{["pdf"]}). PDF is HTML\textrightarrow{}Chromium; \texttt{.tex} is a self-contained pdflatex source you can edit and recompile; \texttt{.docx} is generated with the docx library for Word / Google Docs editing. All three share the same tailored content. URLs persist onto the application row's \texttt{rendered\_files} JSON. The visa-leakage rail runs against every output before any file is written. Files land under \fpath{/files/pdfs/}, \fpath{/files/tex/}, \fpath{/files/docx/}.
...
\begin{description}[leftmargin=0pt]
\item[\tool{get\_tracker}] Full pipeline snapshot as JSON. Optional \texttt{status} filter. Mirrors the dashboard at \texttt{/}.
...
\begin{notebox}
When \env{MCP\_JSA\_VISA\_SCORING=false}, all three visa tools are hidden from \texttt{tools/list} and the score formula renormalizes to \texttt{0.6 \(\cdot\) resume + 0.4 \(\cdot\) taste}. See \S\ref{sec:custom}.
\end{notebox}
...
On the next \texttt{reseed} (CLI \texttt{npx job\_ops-mcp reseed} or the \tool{reseed\_career\_packet} tool) these auto-populate Section 2 \textemdash{} one bullet per archetype. The field is backwards-compatible: omit it entirely and Section 2 keeps the editable template placeholders exactly as before.
...
\item Prepends a ``VISA SCORING DISABLED'' override block to the top of the \texttt{mcp-jsa://modes/rubric} resource.
...
The loader checks \texttt{\$MCP\_JSA\_TEMPLATE\_DIR} first, so a \fpath{default/} directory in your themes dir overrides the bundled default. To make a custom theme the implicit default for every \tool{render\_pdf} call (instead of passing \texttt{template=...}):

\begin{lstlisting}[language=bashplus]
export MCP_JSA_DEFAULT_TEMPLATE=compact
\end{lstlisting}

Hard rules still apply regardless of theme: the visa-leakage scan runs on \texttt{cover\_body} \emph{before} substitution, so a custom template cannot inject visa/work-auth language. A theme that omits a placeholder silently drops that section (graceful degradation). A theme missing \verb|\documentclass| or \verb|\begin{document}| produces an error that quotes the theme name + file path \textemdash{} pdflatex's own backtrace never reaches the user.
...
\begin{tipbox}
Sampling and elicitation are server\(\rightarrow\)client \emph{requests}, so they also require a transport that carries bidirectional traffic: \textbf{stdio} (e.g. Claude Desktop). The stateless streamable-HTTP transport answers one POST with one JSON reply and cannot deliver a server-initiated request, so over HTTP both features gate \emph{off} and the server falls back \textemdash{} BYO-key scoring, and the file/argument config paths. The gate checks the transport too, so HTTP clients fall back cleanly instead of hanging.
\end{tipbox}

\subsection*{MCP sampling \textemdash{} no LLM key required}

The \texttt{api}-path scorers (\tool{batch\_evaluate}, \tool{evaluate\_job} \texttt{mode="api"}) prefer \textbf{sampling}: the server issues a \texttt{sampling/createMessage} request to the connected client and the \emph{client's own model} produces the completion. Same rubric, same strict-JSON contract as the BYO-key path \textemdash{} so there is nothing to collapse at the contract level; one \texttt{Completer} abstraction (\fpath{src/core/scoring.ts}) backs both, and \tool{evaluate\_job}/\tool{batch\_evaluate} report \texttt{scored\_via: "sampling"} or \texttt{"api"}.

\begin{itemize}
\item \textbf{Selection order:} sampling (if the client advertises it and \texttt{MCP\_JSA\_SAMPLING}\,$\neq$\,\texttt{false}) \(\rightarrow\) BYO key (\texttt{MCP\_JSA\_LLM\_PROVIDER} + key) \(\rightarrow\) otherwise \tool{evaluate\_job} \texttt{mode="chat"} still works (the chat scores it).
\item \textbf{Cost:} sampling runs on the client's model, so the client bears the cost. \tool{cost\_estimate} still records every sampling call and flags it \texttt{cost\_borne\_by: client} with a \$0 server estimate, so the total is not misread.
\end{itemize}
...
See \S\ref{sec:security} below for the full table. In short: localhost-only stays open and frictionless; binding to anything else requires \texttt{MCP\_JSA\_AUTH\_TOKEN} or the server refuses to start.
...
Binding to a non-localhost host without \texttt{MCP\_JSA\_AUTH\_TOKEN} makes the server \textbf{refuse to boot} (default-deny). When a token is set, \texttt{/mcp}, \texttt{/files/*}, and the dashboard require a bearer token; only \texttt{/healthz} + discovery metadata stay open. Resume PDFs, LinkedIn connections, and H1B data never reach an unauthenticated remote caller. See \S\ref{sec:security}.
\end{warnbox}
...
\begin{description}
\item[\textit{``Chromium isn't installed''} on first \texttt{start}] \texttt{npx job\_ops-mcp start} auto-installs Chromium on first run (~150 MB, ~30 s). If you skipped it via \cmd{--skip-chromium-check} or the install errored, run \cmd{npx playwright install chromium} manually and re-try.

\item[\textit{``Tool not found''} from the chat client] Your client connected before the server had registered tools. Check the connection mode \textemdash{} for Claude Desktop, the \texttt{command: npx} block should spawn the server inline with the \flag{--stdio} flag; for LibreChat, the server must already be running on the URL you specified. Run \cmd{npx job\_ops-mcp doctor} to confirm.
...
\item[\textit{LLM tools error with ``No LLM provider configured''}] Set \env{MCP\_JSA\_LLM\_PROVIDER} + the matching API key. The default \texttt{chat} mode of every tool works without one \textemdash{} only \tool{batch\_evaluate} and \texttt{mode: api} need a key.

\item[\textit{Migrations didn't run}] The server applies pending migrations on first DB open. If the \fpath{data/} directory is on a read-only filesystem, the boot will error. Set \env{MCP\_JSA\_DATA\_DIR} to a writable path and re-run.

\item[\textit{Doctor says ``cv.md missing''} but it's in the directory] You probably launched from a different CWD than where \cmd{init} ran. Set \env{MCP\_JSA\_PROJECT\_ROOT} to the absolute path, or \cmd{cd} into the right directory before \cmd{start}.
\end{description}

\subsection*{Conceptual FAQ}

\begin{description}
\item[Does this work without visa data?] Yes. \env{MCP\_JSA\_VISA\_SCORING=false} drops the visa surface entirely \textemdash{} formula renormalizes, tools hidden, columns stripped. Use the system exactly as documented; only the visa-specific bits go quiet.
...
\item[Where does the SQLite file go?] By default, \texttt{<project\_root>/data/mcp-jsa.db}. Override with \env{MCP\_JSA\_DATA\_DIR}.
...
job\_ops-mcp \textbullet{} v0.6.0 \textbullet{} MIT \textbullet{} \url{https://github.com/mohith-das/job_ops-mcp}
\end{flushright}
