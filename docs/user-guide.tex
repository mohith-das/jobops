...
\begin{tabular}{rl}
\textsf{\small\bfseries Version} & \texttt{0.7.0} \\
...
One process. Three planes. Six MCP resources and 37 tools.
...
Restart Claude Desktop. The 33--37 tools should appear in the tool picker.
...
\subsection*{Optional: enable the LLM API path}
...
If you want server-side scoring (the \tool{batch\_evaluate} tool, or \texttt{mode: api} on tools that accept it), set one of:
...
\tool{cost\_estimate} reports your cumulative spend per provider per tool.
...
37 tools, grouped by what they do. For each: what it does, key parameters, what it returns. The default mode for tools that take one is \texttt{chat} \textemdash{} the chat client is the brain; \texttt{api} mode is the LLM-direct path and needs an API key.
