...
\begin{itemize}
\item \textbf{Boot in 2 seconds.} \cmd{npx @mohith\_das/jobops start} \textemdash{} no docker compose, no migrations to run by hand, no init container.
...
\begin{tipbox}
The package is scoped (\cmd{@mohith\_das/jobops}) but the installed command is just \cmd{jobops} \textemdash{} once it's on your \texttt{PATH} (global install) you can drop the \cmd{npx @mohith\_das/jobops} prefix and run \cmd{jobops start}, \cmd{jobops doctor}, etc. directly.
\end{tipbox}
...
After \cmd{npx @mohith\_das/jobops init} from a clean directory you'll have:
...
Boot the server separately (\cmd{npx @mohith\_das/jobops start}), then restart LibreChat.
...
devices, also set \env{JOBOPS\_HOST=0.0.0.0}. \cmd{npx @mohith\_das/jobops doctor} prints the
...
On the next \texttt{reseed} (CLI \texttt{npx @mohith\_das/jobops reseed} or the \tool{reseed\_career\_packet} tool) these auto-populate Section 2 \textemdash{} one bullet per archetype. The field is backwards-compatible: omit it entirely and Section 2 keeps the editable template placeholders exactly as before.
...
\begin{description}
\item[\textit{``Chromium isn't installed''} on first \texttt{start}] \texttt{npx @mohith\_das/jobops start} auto-installs Chromium on first run (~150 MB, ~30 s). If you skipped it via \cmd{--skip-chromium-check} or the install errored, run \cmd{npx playwright install chromium} manually and re-try.

\item[\textit{``Tool not found''} from the chat client] Your client connected before the server had registered tools. Check the connection mode \textemdash{} for Claude Desktop, the \texttt{command: npx} block should spawn the server inline with the \flag{--stdio} flag; for LibreChat, the server must already be running on the URL you specified. Run \cmd{npx @mohith\_das/jobops doctor} to confirm.
