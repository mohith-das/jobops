...
\begin{tabular}{rl}
\textsf{\small\bfseries Version} & \texttt{0.9.2} \\
...
\begin{itemize}
\item \textbf{Selection order:} sampling (only if the client advertised \texttt{sampling} \emph{and} \texttt{MCP\_JSA\_SAMPLING}\,$\neq$\,\texttt{false}) \(\rightarrow\) BYO key (\texttt{MCP\_JSA\_LLM\_PROVIDER} + key) \(\rightarrow\) otherwise \tool{evaluate\_job} \texttt{mode="chat"} still works (the chat scores it). On a client without sampling (e.g. Claude Desktop), set the BYO key for \tool{batch\_evaluate} / \texttt{mode="api"}.
