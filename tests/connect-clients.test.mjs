...
function runConnect(env = {}) {
  const r = spawnSync(process.execPath, [CLI, 'connect'], {
    encoding: 'utf-8',
    env: { ...process.env, JOBOPS_AUTH_TOKEN: '', JOBOPS_PUBLIC_BASE_URL: '', ...env },
  });
...
test('connect emits a config block for every client, all pointing at ONE shared URL', () => {
  ...
  assert.match(out, new RegExp(`claude mcp add --transport http jobops ${url}`));          // Claude Code
...
test('connect with JOBOPS_AUTH_TOKEN embeds the bearer token in every HTTP config', () => {
  const out = runConnect({ JOBOPS_AUTH_TOKEN: 'tok-abc123' });
  ...
  assert.match(out, /bearer_token_env_var = "JOBOPS_AUTH_TOKEN"/);
});

test('connect respects JOBOPS_PUBLIC_BASE_URL (remote / Tailscale host)', () => {
  const out = runConnect({ JOBOPS_PUBLIC_BASE_URL: 'http://ampere.tail1234.ts.net:7891', JOBOPS_AUTH_TOKEN: 'tok' });
...
test('status command points at a non-running server → clear failure', () => {
  const r = spawnSync(process.execPath, [CLI, 'status', '--url', 'http://127.0.0.1:1'], {
    encoding: 'utf-8',
    env: { ...process.env, JOBOPS_AUTH_TOKEN: '' },
  });
