...
Below are the two configs you most likely want to paste by hand. The stdio config is the \emph{single-client} alternative: it spawns a private server inside Claude Desktop rather than connecting to the shared one (next to a running shared server it also needs its own \env{MCP\_JSA\_PORT}, or its file server fails with \texttt{EADDRINUSE}).
